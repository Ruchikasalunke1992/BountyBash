...
class GameScene2: SKScene, SKPhysicsContactDelegate {
    ...
    let car = SKSpriteNode(imageNamed: "spaceShuttle.png")
    ...
    func addBounty2(){
        ...
        // Where to create boutny along the x-axis
        let minX = bounty2.size.width/2 + 15
        let maxX = self.frame.size.width - 15 - bounty2.size.width/2
        let rangeX = maxX - minX
        let position:CGFloat = CGFloat(Int(arc4random_uniform(UInt32(UInt(rangeX ))))) + minX
        
        // Bring bounty of screen
        bounty2.position = CGPointMake(position, self.frame.size.height+bounty2.size.height)
        
        self.addChild(bounty2)
  
        // boutny Animation Duration
    ...
    func addbg(){
        ...
        var backgroundTexture = SKTexture(imageNamed: "space.png")
    ...
    func addEnemy(){
        ...
        // Create enemy sprite
        var enemy:SKSpriteNode = SKSpriteNode(imageNamed: "enemyshuttle.png")
        enemy.frame
        ...
        let minX = enemy.size.width/2 + 15
        let maxX = self.frame.size.width - 15 - enemy.size.width/2
        let rangeX = maxX - minX
        let position:CGFloat = CGFloat(Int(arc4random_uniform(UInt32(UInt(rangeX ))))) + minX
    ...
    override func update(currentTime: CFTimeInterval) {
        ...
        if let touch = lastTouch {
            ...
            car.physicsBody?.applyImpulse(CGVectorMake(touch.x - car.position.x, 0))
        }
